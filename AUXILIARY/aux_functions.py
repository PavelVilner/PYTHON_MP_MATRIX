...
def z0_from_args(N, *args):
    if not args:
        return mp.sqrt(mp.mpf("50"))*mp.diag(mp.ones(1,N))
...
def deg(local_in):
    ...
    '''
    if is_scalar(local_in):
        return local_in / mp.pi * mp.mpf("180")
    else:
        return apply_to_iterable(local_in, lambda x: x / mp.pi * mp.mpf("180"))
...
def dB20(local_in):
    ...
    '''
    if is_scalar(local_in):
        return mp.mpf("20") * mp.log10(mp.fabs(local_in))
    else:
        raise NotImplemented()
...
def dB10(local_in):
    ...
    '''
    if is_scalar(local_in):
        return mp.mpf("10") * mp.log10(mp.fabs(local_in))
    else:
        raise NotImplemented()
...
#             local_list = []
#             for n in range(args[0][0]):
#                 local_list.append(make_N_dim_list(args[0][1:len(args[0])]))
            return local_list
    except Exception as E:
        raise E
    
def mpf_to_str(num, symbols):
    num = mp.re(num)
    if symbols <= 3:
    ...
    num_char = mp.floor(mp.log10(mp.fabs(num)))
    num_val = mp.fabs(num) / (mp.mpf("10")**num_char)
...
def mpfi_to_str(num, symbols):
    local_str = mpf_to_str(mp.im(num), symbols-1)
...
def mpc_to_str(num, symbols):
    ...
    abs_re_str = mpf_to_str(mp.fabs(mp.re(num)), int((symbols-5)/2+1))[1:]
    abs_im_str = mpf_to_str(mp.fabs(mp.im(num)), int((symbols-5)/2+1))[1:]
    if mp.re(num) >= 0:
        num_re_sign = " "
    else:
        num_re_sign = "-"
